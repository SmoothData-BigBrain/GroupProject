...
### 2. Preprocessing 
We decided to remove columns we thought to be redundant for analysis or were related to other columns. For instance, we removed FlightDate because of the Year, Month, and DayofWeek columns. For missing data, columns with more than 10% of null values were excluded from our analysis. Columns that had 1-3% of missing values were Tail_Number, DepTime, DepDelay, DepDelayMinutes, DepDel15, DepartureDelayGroups, WheelsOff, TaxiOut, ArrTime, WheelsOn, TaxiIn, ActualElapsedTime, ArrDelay, ArrDelayMinutes, ArrDel15,  ArrivalDelayGroups, and AirTime. Their missing values were handled by simply removing the missing rows as they accounted for only 0.27% of observations. Additionally, a statistical summary was created measuring the following statistics for numerical features: count, mean, standard deviation, minimum, maximum, and the 25th, 50th (median), and 75th percentile.

Feature engineering was performed by creating new columns, encoding categorical variables, and reducing the label column. For feature expansion, Route was created by combining the Origin and Dest columns. Avg_speed_mph (Distance/AirTime * 60) and num_flights (the count of each route) were created to aid in model performance. Categorical variables were indexed using StringIndexer to convert them to numeric values to be used in the Random Forest model. Initially, our target column contained 16 label categories. As part of preprocessing, we reduced this to 4 balanced categories, both for model preparation and for simplicity.

To conduct visual exploration, we posed several analytical questions related to flight delays and visualized relevant features to explore potential insights. Each question is accompanied by a corresponding plot and a brief discussion. This exploratory step was crucial for identifying useful features and greatly aided in our final feature selection process.

### 3. Model 1: Random Forest Classifier
We selected random forest classification as our model because of its robustness, versatility, and high accuracy ceiling. Of the many viable target variables, we decided to use ArrivalDelayGroups, which measures flights' arrival delay intervals every 15 minutes.
